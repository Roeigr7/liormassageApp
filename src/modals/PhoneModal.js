...
const PhoneModal = () => {
	...
	const phoneHandler = () => {
		phoneValidator(phone) ? updateUser(state.userId, state.token, "phone", phone) : setError(true);
	};
	const closeModal = () => {
		setVisible(false);
		...
		<View style={styles.containerbg}>
			...
			>
				<View style={styles.container}>
					...
					/>
					{error && <Text style={styles.errortext}>נא להקיש מספר טלפון בעל 10 ספרות {"\n"} שמתחיל בספרה 0</Text>}
					<AnimatedButton onPress={phoneHandler} buttonStyle={styles.animatedbtn} text="אישור" />
				</View>
			</Modal>
		</View>
	);
